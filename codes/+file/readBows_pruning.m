...
% Examples:
%   - [bows, dict] = file.readBows_pruning('../../jsmf-raw/dataset/real_raw/docword.nips.txt',  '../../jsmf-raw/dataset/real_raw/vocab.nips.txt', '../../jsmf-raw/dataset/standard.stops', 5000, 5, '../dataset/nips_N-5000');
%   - bows = file.readBows('../../jsmf-raw/dataset/real_bows/nips_N-5000_train.bows', 0.5);
%

%%
% Main: readBows_pruning()
%
% Inputs:
...
%   - pruning: parameter that determines how much vocab to prune based on document frequency
%     Note: pruning = 1 does not prune any vocab words
%
% Intermediates:
...
%  
function [bows, dict] = readBows_pruning(bows_filename, dict_filename, stop_filename, N, min_objects, output_filename, pruning)
    % Print out the initial status.
    fprintf('[file.readBows_pruning] Start reading Bag-of-words dataset...\n');    
    startTime = tic;
    ...
    idfs = log(M ./ dfs);    
    %idfs(dfs > M/2) = 0; 
    idfs(dfs > M*pruning) = 0;
    prunedObjects = find(dfs > M*pruning);
    prunedIndices = ismember(bows(:, 2), prunedObjects);
    bows = bows(~prunedIndices, :);
