...
%%
% Inner: allGivenH_vocab()
%
% Remark: 
%   - This function evaluates every possible metric based on the
%   compressed co-occurrence information.
%
function [value, stdev] = allGivenH_vocab(S, B, A, Btilde, H, C_rowSums, withTitle)
    % Decide whether or not printing out metric titles.
    if nargin < 7
        withTitle = 0;
    end
...
%%
% Inner: allGivenYE_vocab()
%
% Remark: 
%   - This function evaluates every possible metric based on the
%   compressed co-occurrence information.
%
function [value, stdev] = allGivenYE_vocab(S, B, A, Btilde, Y, E, C_rowSums, ratio, withTitle)
    % Decide whether or not printing out metric titles.
    if nargin < 9
        withTitle = 0;
    end
