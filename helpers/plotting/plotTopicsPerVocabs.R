...
for (i in 1:numDatasets)
{
  ...
  for (j in 1:numDiffVocabs)
  {
    ...
    metricTable[, 19] <- log(metricTable[, 19], base=10)
    ...
    # Save only the specific number of topics of our focus.
    metricTables[[i]][[j]] <- metricTable[focuses, ]
    
    # Add one more column indicating the experiment detail.
    metricTables[[i]][[j]]$Vocabs <- vocab
  }
}
...
for (i in 1:numDatasets)
{
  ...
  plots[[i]] <- ggplot(subset(figures[[i]], Metric %in% c("FullRecovery", "PartRecovery", "G2Separability", "GsSeparability", "Legality", "Validity", "Approximation", "Dominancy", "Entropy", "Specificity", "Dissimilarity", "BasisQuality", "Sparsity", "RectifyTime", "FactorizeTime")), aes(Vocabs, value, color=Topics, shape=Topics, linetype=Topics))
  ...
  ggsave(outputFile, height=2.3, width=22)  
}  
  
pdf(paste(outputBase, ".pdf", sep=""), height=(2.1*numDatasets), width=22)
