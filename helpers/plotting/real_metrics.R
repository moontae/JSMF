...
datasets <- c("nips_N-5000", "nytimes_N-15000", "movies_N-10000", "songs_N-10000")
methods <- c("AP", "ENN-randEig", "ENN-trunEig", "PALM")
opt <- "activeSet"
...
dataLabels <- c("NIPS (# documents M=1,348  /  # vocabulary N=5k  /  average document length=380.5)", "NYTimes (# documents M=263,325  /  # vocabulary N=15k  /  average document length=204.9)", "Movies (# documents M=63,041  /  # vocabulary N=10k  /  average document Length=142.8)", "Songs (# documents M=14,653  /  # vocabulary N=10k  /  average document length=119.2)")
methodNames <- c("AP", "ENN-randEig", "ENN-trunEig", "PALM")
...
lineTypes = c("solid", "solid", "solid", "dashed")
...
for (i in 1:numDatasets)
{
  ...
  ## save the plot 
  outputFile <- paste("real_metrics", "-", dataName, ".pdf", sep="")
  ggsave(outputFile, height=2.3, width=15)  
}  
  
pdf(paste("real_metrics", ".pdf", sep=""), height=(2.1*numDatasets), width=15)
